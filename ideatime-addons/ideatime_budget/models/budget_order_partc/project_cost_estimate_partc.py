...
class ProjectCostEstimatePartC(models.Model):
    _name = 'project.cost.estimate.partc'
    _description = 'Project Cost Estimate Part C'
    ...
    budget_calculator_line = fields.One2many('budget.calculator.line', 'project_cost_estimate_partc_id')
    ...
    usage_type = fields.Selection(
        ...
        default='self')
    budget_batch_line_id = fields.One2many('budget.batch.line', 'budget_partC_id')
    ...
    def open_batch(self):
        ...
        view_id = self.env.ref('ideatime_budget.view_budget_partC_batch').id
        return {
            'name': ('Batch Information'),
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'project.cost.estimate.partc',
            'views': [(view_id, 'form')],
            'view_id': view_id,
            'target': 'new',
            'res_id': self.ids[0],
    ...
    def expense_calculate_cbm(self):
        action_ctx = dict(self.env.context)
        view_id = self.env.ref('ideatime_budget.view_partc_expense_line_cbm_calc').id
