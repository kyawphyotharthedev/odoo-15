...
var PecOverviewReport = stock_report_generic.extend({
    events: {
...
//            self.update_cp();
    ...
    get_operations: function(event) {
      ...
      return this._rpc({
              model: 'report.mrp.pec_report_bom_structure',
    ...
    _onClickPrint: function (ev) {
        ...
        framework.blockUI();
        var reportname = 'mrp.pec_report_bom_structure?docids=' + this.given_context.active_id +
        ...
        var action = {
            ...
            'report_file': 'mrp.pec_report_bom_structure',
        };
...
//        $(ev.currentTarget).toggleClass('o_mrp_bom_unfoldable o_mrp_bom_foldable fa-caret-down fa-caret-right');
    },
    _onClickFold: function (ev) {
//        this._removeLines($(ev.currentTarget).closest('tr'));
        this._hideLines($(ev.currentTarget).closest('tr'));
    ...
    _showLines: function($el){
        var self = this;
        var activeID = $el.data('id');
        var rows = this.$('tr[parent_id='+ activeID +']');
        for (let row of rows) {
            var $p = self.$(row);
...
core.action_registry.add('pec_overview_report', PecOverviewReport);
export default PecOverviewReport;
