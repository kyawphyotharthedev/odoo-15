...
class ProjectAnalysisXlsReport(models.AbstractModel):
    _name = 'report.ideatime_project.p_p_analysis_report_id'
    _inherit = 'report.report_xlsx.abstract'
    _description = "Project Estimate Analysis"

    def get_display_data(self, objects):
        summary_line = []
        for line in objects.project_cost_estimate_line:
    ...
    def get_order_line_display_data(self, objects):
        ...
        for o_line in objects.project_order_line:
            all_dir_order_line = self.env['projectdir.material.cost'].search([('project_line_id', 'in', o_line.ids)])

            for line in all_dir_order_line:
                if line.project_line_id:
                    for dir_line in line.project_line_id:
    ...
    def get_indir_order_line_display_data(self, objects):
        ...
        for o_line in objects.project_order_line:
            all_dir_order_line = self.env['projectindir.material.cost'].search([('project_line_id', 'in', o_line.ids)])
            for line in all_dir_order_line:
                if line.project_line_id:
                    for indir_line in line.project_line_id:
    ...
    def get_dir_labour_order_line_display_data(self, objects):
        ...
        for o_line in objects.project_order_line:
            all_dir_order_line = self.env['projectdir.labour.cost'].search([('project_line_id', 'in', o_line.ids)])
            for line in all_dir_order_line:
                if line.project_line_id:
                    for dirlabour_line in line.project_line_id:
    ...
    def get_thirdparty_order_line_display_data(self, objects):
        ...
        for o_line in objects.project_order_line:
            all_dir_order_line = self.env['project.thirdparty.cost.contractor'].search(
                [('project_line_id', 'in', o_line.ids)])
            for line in all_dir_order_line:
                if line.project_line_id:
                    for thirdparty_con_line in line.project_line_id:
    ...
    def get_subthirdparty_order_line_display_data(self, objects):
        ...
        for o_line in objects.project_order_line:
            all_dir_order_line = self.env['project.thirdparty.cost.subcontractor'].search(
                [('project_line_id', 'in', o_line.ids)])

            for line in all_dir_order_line:
                if line.project_line_id:
                    for subthirdpary_line in line.project_line_id:
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        sheet.merge_range(display_row, 1, display_row, 3, objects.project_name, format8)
        display_row += 1
        ...
        sheet.write(display_row, 3, objects.user_id.name or '', format7)
        ...
        sheet.merge_range(display_row, 1, display_row, 3, objects.cate_group_id.name or '', format7)
        display_row += 1
        sheet.write(display_row, 0, 'Service sector', format6)
        sheet.merge_range(display_row, 1, display_row, 3, objects.cate_sector_id.name or '', format7)
        display_row += 1
        sheet.write(display_row, 0, 'Project site', format6)
        sheet.merge_range(display_row, 1, display_row, 3, objects.project_site or '', format7)
        ...
        display_row += 1
        sheet.write(display_row, 0, 'Indirect Materials Cost', format6)
        ...
        display_row += 1
        sheet.write(display_row, 0, 'Direct Labour Cost', format6)
        ...
        display_row += 1
        sheet.write(display_row, 0, 'Thirdparty Cost Contractor', format6)
        ...
        display_row += 1
        sheet.write(display_row, 0, 'Thirdparty Cost subcontractor', format6)
        ...
        sheet.insert_chart('D16', draw_chart, {'x_scale': 0.7, 'y_scale': 1.5})
