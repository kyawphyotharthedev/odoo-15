...
class ReportExpenseBudgetOverview(models.AbstractModel):
    _name = 'budget.expense.overview.report'
    _description = 'Budget Expense Overview Report'
    ...
    @api.model
    def get_html(self, expense_id=False):
        res = self._get_report_data(expense_id=expense_id)
        res['lines']['report_type'] = 'html'
        res['lines'] = self.env.ref('ideatime_expense.report_budget_expense_overview')._render({'data': res['lines']})
        return res

    @api.model
    def _get_report_data(self, expense_id):
        lines = {}
        lines = self._get_budget_line(expense_id, level=1)
        return {
            'lines': lines,
        }

    def _get_budget_line(self, expense_id=False, line_id=False, level=False):

        expense_id= self.env['hr.expense'].browse(expense_id)
        estimate_line = self._get_budget_data_line(expense_id.budget_id, 0)
        account_payment = self._get_account_payment(expense_id.budget_id)
        lines = {
            'budget': expense_id.budget_id,
            'level': level or 0,
    ...
    def _get_budget_data_line(self, expense_id, level):
        res = self._get_budget_detail_line(expense_id, level, {})
        line = []
        for data in res:
            line.append(data)
        return line
