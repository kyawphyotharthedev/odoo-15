odoo.define("account_dynamic_reports.DynamicTbMain", function (require) {
  ...
  var DynamicFrMain = AbstractAction.extend({
    ...
    view_gl: function (event) {
      ...
      var domains = {
        ...
        initial_balance:
          self.filter_data.rtype == "CASH" || self.filter_data.rtype == "PANDL"
            ? false
            : true,
      };
      var context = {};
      ...
      self
        ...
        .then(function (datas) {
          ...
          _.each(self.account_data, function (k, v) {
            ...
            k.ldate = field_utils.format.date(
              field_utils.parse.date(k.ldate, {}, { isUTC: true })
            );
            _.each(k.lines, function (ks, vs) {
              ks.debit = self.formatWithSign(
                ks.debit,
                formatOptions,
                ks.debit < 0 ? "-" : ""
              );
              ks.credit = self.formatWithSign(
                ks.credit,
                formatOptions,
                ks.credit < 0 ? "-" : ""
              );
              ks.balance = self.formatWithSign(
                ks.balance,
                formatOptions,
                ks.balance < 0 ? "-" : ""
              );
              ks.ldate = field_utils.format.date(
                field_utils.parse.date(ks.ldate, {}, { isUTC: true })
              );
            });
          });
          if (initial_render) {
            self.$(".py-control-panel").html(
              QWeb.render("FilterSection", {
                filter_data: datas[0],
              })
            );
            self.$el.find("#date_from").datepicker({ dateFormat: "dd-mm-yy" });
            ...
            self.$el
              ...
              .val(["include_details", "initial_balance"])
              .trigger("change");
          ...
          self.$(".py-data-container-orig").html(
            QWeb.render("DataSection", {
    ...
    fetch_move_lines_by_page: function (event) {
      ...
      self.gl_lines_by_page(offset, account_id).then(function (datas) {
        ...
        $(event.currentTarget)
          ...
          .parent()
          .parent()
          .find(".py-mline-table-div")
          .remove();
        $(event.currentTarget)
          .parent()
          .parent()
          .find("a")
          .css({ "background-color": "white", "font-weight": "normal" });
        $(event.currentTarget)
          .parent()
          .parent()
          .after(
  ...
  var DynamicPaMain = AbstractAction.extend({
        ...
        .then(function (datas) {
          ...
          _.each(self.ageing_data, function (k, v) {
            ...
            for (var z = 0; z < self.period_list.length; z++) {
              k[self.period_list[z]] = self.formatWithSign(
                k[self.period_list[z]],
                formatOptions,
                k[self.period_list[z]] < 0 ? "-" : ""
              );
            }
            k.total = self.formatWithSign(
              k.total,
              formatOptions,
              k.total < 0 ? "-" : ""
            );
    ...
    fetch_move_lines_by_page: function (event) {
      ...
      self.ageing_lines_by_page(offset, partner_id).then(function (datas) {
        ...
        _.each(account_data, function (k, v) {
          ...
          k.range_0 = self.formatWithSign(
            k.range_0,
            formatOptions,
            k.range_0 < 0 ? "-" : ""
          );
          k.range_1 = self.formatWithSign(
            k.range_1,
            formatOptions,
            k.range_1 < 0 ? "-" : ""
          );
          k.range_2 = self.formatWithSign(
            k.range_2,
            formatOptions,
            k.range_2 < 0 ? "-" : ""
          );
          k.range_3 = self.formatWithSign(
            k.range_3,
            formatOptions,
            k.range_3 < 0 ? "-" : ""
          );
          k.range_4 = self.formatWithSign(
            k.range_4,
            formatOptions,
            k.range_4 < 0 ? "-" : ""
          );
          k.range_5 = self.formatWithSign(
            k.range_5,
            formatOptions,
            k.range_5 < 0 ? "-" : ""
          );
          k.range_6 = self.formatWithSign(
            k.range_6,
            formatOptions,
            k.range_6 < 0 ? "-" : ""
          );
          k.date_maturity = field_utils.format.date(
            field_utils.parse.date(k.date_maturity, {}, { isUTC: true })
          );
        });
        $(event.currentTarget)
          .parent()
    ...
    update_with_filter: function (event) {
      ...
      if (
        (parseInt(output.bucket_1) >= parseInt(output.bucket_2)) |
        (parseInt(output.bucket_2) >= parseInt(output.bucket_3)) |
        (parseInt(output.bucket_3) >= parseInt(output.bucket_4)) |
        (parseInt(output.bucket_4) >= parseInt(output.bucket_5))
      ) {
  ...
  return {
    DynamicFrMain: DynamicFrMain,
    DynamicGlMain: DynamicGlMain,
    DynamicPaMain: DynamicPaMain,
    DynamicPlMain: DynamicPlMain,
    DynamicTbMain: DynamicTbMain,
  };
});
